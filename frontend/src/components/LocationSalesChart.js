...
const LocationSalesChart = () => {
  ...
  const fetchLocationSalesData = async () => {
    try {
      ...
      const response = await dashboardAPI.getLocationSales(dateRange);
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="ml-5 w-0 flex-1">
              <dl>
                ...
                <dd className="text-xs text-gray-500">
                  {topLocation ? formatCurrency(topLocation.total_revenue) : ''}
                </dd>
              </dl>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white shadow rounded-lg p-6">
          <h4 className="text-lg font-medium text-gray-900 mb-4">Revenue by Location</h4>
        ...
        <div className="bg-white shadow rounded-lg p-6">
          <h4 className="text-lg font-medium text-gray-900 mb-4">Sales Distribution</h4>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
                data={locationData}
      ...
      <div className="bg-white shadow rounded-lg p-6">
        <h4 className="text-lg font-medium text-gray-900 mb-4">Location Performance Details</h4>
              ...
              {locationData.map((location, index) => {
                ...
                return (
                  <tr key={index} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {formatCurrency(location.paid_revenue)}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {formatCurrency(location.avg_sale_amount)}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
      ...
      {topItemsByLocation.length > 0 && (
        <div className="bg-white shadow rounded-lg p-6">
          <h4 className="text-lg font-medium text-gray-900 mb-4">Top Selling Items by Location</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {topItemsByLocation.map((locationData, index) => (
              <div key={index} className="border border-gray-200 rounded-lg p-4">
                <h5 className="font-medium text-gray-900 mb-3">{locationData.location_name}</h5>
                  ...
                  {locationData.items.slice(0, 5).map((item, itemIndex) => (
                    <div key={itemIndex} className="flex justify-between items-center text-sm">
                      ...
                      <div className="text-right">
                        <div className="text-gray-900 font-medium">{item.total_quantity_sold} sold</div>
                        <div className="text-gray-500">{formatCurrency(item.total_revenue)}</div>
                      </div>
                    </div>
                  ))}
                </div>
