...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      setLoading(true);
      const [overviewRes, activitiesRes, topItemsRes, lowStockRes, settingsRes] = await Promise.all([
  ...
  const statCards = [
    ...
    },
    {
      ...
      value: formatCurrency(stats?.netInvestments || 0, settings?.currency || 'LKR', 0),
      icon: CurrencyDollarIcon,
      color: 'bg-green-500',
      subtext: `${formatCurrency(stats?.totalInvestments || 0, settings?.currency || 'LKR', 0)} In, ${formatCurrency(stats?.totalWithdrawals || 0, settings?.currency || 'LKR', 0)} Out`,
    },
    {
      name: 'Total Actual Inventory Value',
      value: formatCurrency(stats?.totalInventoryBuyValue || 0, settings?.currency || 'LKR', 0),
      icon: CubeIcon,
      color: 'bg-purple-500',
    ...
    },
    {
      ...
      value: formatCurrency(stats?.totalInventorySellValue || 0, settings?.currency || 'LKR', 0),
      icon: CubeIcon,
      color: 'bg-indigo-500',
    ...
    },
    {
      ...
      color: 'bg-orange-500',
      subtext: `${formatCurrency(stats?.totalRevenue || 0, settings?.currency || 'LKR', 0)} Revenue`,
    },
   ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        {statCards.map((stat) => (
          <div key={stat.name} className="bg-white dark:bg-gray-800 overflow-hidden shadow dark:shadow-gray-700 rounded-lg">
            <div className="p-5">
                      ...
                      <div className="relative flex space-x-3">
                        ...
                        <div className="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                          ...
                          <div className="text-right text-sm whitespace-nowrap text-gray-500 dark:text-gray-400">
                            <p>{formatCurrency(activity.amount, settings?.currency || 'LKR', 2)}</p>
                ...
                {lowStockItems.map((item) => (
                  <div key={item.id} className="flex items-center justify-between p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">{formatCurrency(item.sell_price, settings?.currency || 'LKR', 2)}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="px-4 py-5 sm:p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="text-center">
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{stats?.todaySales || 0}</p>
              <p className="text-sm text-gray-500 dark:text-gray-400">Sales Today</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-green-600 dark:text-green-400">{formatCurrency(stats?.todayRevenue || 0, settings?.currency || 'LKR', 0)}</p>
              <p className="text-sm text-gray-500 dark:text-gray-400">Revenue Today</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{formatCurrency(stats?.totalInventorySellValue || 0, settings?.currency || 'LKR', 0)}</p>
